...
class PickTransactionsUpdate extends Component {
  constructor(props) {
    super(props);
  ...
  handleUpdatePickTransactions = async () => {
    const {
      id,
      STK,
      PORT,
    ...
    await api.updatePickTransactionsById(id, payload).then(res => {
  ...
  componentDidMount = async () => {
    const { id } = this.state;
    const picktransactions = await api.getPickTransactionsById(id);

    this.setState({
      STK: picktransactions.data.data.STK,
      PORT: picktransactions.data.data.PORT,
      SYM: picktransactions.data.data.SYM,
      DATE: picktransactions.data.data.DATE,
      SHARES: picktransactions.data.data.SHARES,
      TOT_COST: picktransactions.data.data.TOT_COST,
      COST_PER_SHARE: picktransactions.data.data.COST_PER_SHARE,
      PRICE: picktransactions.data.data.PRICE,
      VALUE: picktransactions.data.data.VALUE,
      TYPE: picktransactions.data.data.TYPE,
      REALIZED_GAIN_LOSS: picktransactions.data.data.REALIZED_GAIN_LOSS,
      NET_PROCEEDS: picktransactions.data.data.NET_PROCEEDS,
      BROKER: picktransactions.data.data.BROKER,
      COMMENT1: picktransactions.data.data.COMMENT1,
      COMMENT2: picktransactions.data.data.COMMENT2,
      COMMENT3: picktransactions.data.data.COMMENT3
    });
  };
  ...
  render() {
    ...
    return (
      <Wrapper>
        ...
        <Button onClick={this.handleUpdatePickTransactions}>
          Update Transaction
        </Button>
...
export default PickTransactionsUpdate;
